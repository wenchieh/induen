...
# given elements in the leaves, where each internal node stores the max of its two children.
# max tree, which can fetch max value quickly
import math
...
class MaxTree:
    ...
    def getMax(self):
        ...
        for i in range(self.height):
            # select one from left child and right child
            cur = (2 * cur + 1) if self.nodes[2 * cur +
                                              1] >= self.nodes[2 * cur + 2] else (2 * cur + 2)
        # return the maximum index and value
    ...
    def changeVal(self, idx, delta):
        ...
        for i in range(self.height):
            cur = (cur - 1) // 2
            nextParent = max(self.nodes[2 * cur + 1], self.nodes[2 * cur + 2])
    ...
    def dump(self):
        print("numLeaves: %d, numBranches: %d, n: %d, nodes: " %
              (self.numLeaves, self.numBranches, self.n))
        cur = 0
